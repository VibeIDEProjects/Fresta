...
def write_ips_txt(groups, providers, asns, min_count, out_path):
    ...
    filtered = [
        g
        for g in groups
        if match_provider(g["name"], providers)
        and match_asn(g["asn"], asns)
    ...
    with open(out_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"# фильтры: providers={providers or '—'} asns={asns or '—'} min_count={min_count}\n"
        )
...
def write_report_md(meta, out_path):
    ...
    a(
        f"- **фильтры**: providers={meta['filters']['providers'] or '—'}, "
    ...
    if meta.get("subnet_count", 0) > 0:
        ...
        a("")
        a(
            f"С плотностью ≥ {meta['filters']['min_subnet_density'] * 100:.0f}%: "
...
def main():
    ...
    p.add_argument(
        "--out-dir",
        default=os.path.join(os.path.dirname(os.path.abspath(__file__)), "twl-data"),
    ...
    p.add_argument(
        "--repo-dir", default=None, help="использовать уже склонированный twl (оффлайн / CI кэш)"
    )
    p.add_argument(
        "--no-git",
        action="store_true",
    ...
    try:
        ...
        log(
            f"распарсил: {len(groups)} ASN, {len(subnets)} /24, {len(verified)} verified IP",
            json_mode=jm,
        )
        ...
        if jm:
            ...
            print(
                f"  subnets.txt         {subnet_count} /24 (плотность ≥ "
                f"{args.min_subnet_density * 100:.0f}%)"
            )
            print("  twl-harvest-report.md")
            print("  meta.json")
            print()
